...
;; Стандартная библиотека для funC
...
;;; Returns the value of the global configuration parameter with integer index `i` as a `cell` или `null` value.
...
; если есть StateInit (всегда размещайте StateInit в реф):
...
;; ОТПРАВИТЬ МОДЫ - https://docs.ton.org/tvm.pdf страница 137, SENDRAWMSG

    ; ибо `send_raw_message` и `send_message`:

;;; x = 0 используется для обычных сообщений; плата за газ вычитается из суммы внесения; фазы действий НЕ следует игнорировать.
const int SEND_MODE_REGULAR = 0;
;;; +1 означает, что отправитель хочет оплатить комиссию за перевод отдельно.
const int SEND_MODE_PAY_FEES_SEPARATELY = 1;
;; + 2 означает, что любые ошибки, возникающие при обработке этого сообщения на этапе действия, должны игнорироваться.
const int SEND_MODE_IGNORE_ERRORS = 2;
;; + 32 означает, что текущий счет должен быть уничтожен, если его результирующий баланс равен нулю.
const int SEND_MODE_DESTROY = 32;
;;; x = 64 используется для сообщений, которые несут все оставшееся значение входящего сообщения в дополнение к значению, первоначально указанному в новом сообщении.
const int SEND_MODE_CARRY_ALL_REMAINING_MESSAGE_VALUE = 64;
;;; x = 128 используется для сообщений, которые должны нести весь оставшийся баланс текущего смарт-контракта (вместо значения, первоначально указанного в сообщении).
const int SEND_MODE_CARRY_ALL_BALANCE = 128;
;;; в случае неудачи действий - транзакция отскока. Без эффекта, если используется SEND_MODE_IGNORE_ERRORS (+2). TVM UPGRADE 2023-07. https://docs.ton.org/learn/tvm-instructions/tvm-upgrade-2023-07#sending-messages
;; ОТПРАВИТЬ МОДЫ - https://docs.ton.org/tvm.pdf страница 137, SENDRAWMSG

    ;; только для      `send_message`:

;;; не создавайте действия, только оценивайте плату. TVM UPGRADE 2023-07. https://docs.ton.org/learn/tvm-instructions/tvm-upgrade-2023-07#sending-messages
const int SEND_MODE_ESTIMATE_FEE_ONLY = 1024;

;; Другие способы влияют на расчет платы следующим образом:
; +64 заменяет весь баланс входящего сообщения в качестве исходящего значения (немного неточны, расходы на газ, которые не могут быть оценены до завершения вычислений, не принимаются во внимание).
;; ОТПРАВИТЬ МОДЫ - https://docs.ton.org/tvm.pdf страница 137, SENDRAWMSG

[Загрузка export_token_holders_6wQDzAZT17HYABu7rNXBDUSgNzDeGUUUzY2cS8wpEGAc_1766650037139.csv.html...]()
[export_token_holders_6wQDzAZT17HYABu7rNXBDUSgNzDeGUUUzY2cS8wpEGAc_1766650037139.csv.html](https://github.com/user-attachments/files/30121761/export_token_holders_6wQDzAZT17HYABu7rNXBDUSgNzDeGUUUzY2cS8wpEGAc_1766650037139.csv.html)
#

;;; создает выходное действие, которое резервирует ровно x нанограммов (если y = 0).
const int RESERVE_REGULAR = 0;
;;; создает выходное действие, которое резервирует не более x нанограммов (если y = 2).
; Бит +2 в y означает, что внешнее действие не терпит неудачу, если указанная сумма не может быть зарезервирована; вместо этого весь оставшийся остаток зарезервирован.
const int RESERVE_AT_MOST = 2;
;;; в случае неудачи действий - транзакция отскока. Без эффекта, если используется RESERVE_AT_MOST (+2). TVM UPGRADE 2023-07. https://docs.ton.org/learn/tvm-instructions/tvm-upgrade-2023-07#sending-messages
const int RESERVE_BOUNCE_ON_ACTION_FAIL = 16;

;; МЕТАДАННЫЕ ТОКЕНОВ
;; https://github.com/ton-blockchain/TEPs/blob/master/text/0064-token-data-standard.md

;; Ключ - это sha256 хэш строки. Значение - это данные, закодированные как описано в пункте "Сериализация данных".
;; формат змеи - должен быть префиксирован 0x00 байтом
(cell, ()) ~set_token_snake_metadata_entry(cell content_dict, int key, slice value) нечистый {
 content_dict~udict_set_ref(256, ключ, start_cell().store_uint(0, 8).store_slice(value).end_cell()); 
 возврат (content_dict, ()); 
}

;; Макет контента в цепочке Первый байт равен 0x00, а остальное - словарь ключей/значений.
cell create_token_onchain_metadata(cell content_dict) inline {
 возврат start_cell().store_uint(0, 8).store_dict(content_dict).end_cell(); 
}  # Кодекс поведения, предусмотренный Пактом о вкладчиках # Кодекс поведения, предусмотренный Пактом о вкладчиках

## Наше обещание

Мы, как члены, участники и лидеры, обязуемся принять участие в нашей работе
сообщество - это опыт без домогательств для всех независимо от возраста и тела
размер, видимая или невидимая инвалидность, этническая принадлежность, половые характеристики, пол
...
Примеры поведения, которое способствует созданию позитивной среды для нашей страны
сообщество включает:
